...
from FAS_remake import is_forbidden_remake

#CAUSALLEARN CODE SLIGHTLY MODIFIED TO CALL MY BACKGROUND METHODS
...
def pc_remake(
    ...
    **kwargs
    ...
    if mvpc:  # missing value PC
        ...
        return mvpc_alg_remake(data=data, node_names=node_names, alpha=alpha, indep_test=indep_test, correction_name=correction_name, stable=stable,
                        uc_rule=uc_rule, uc_priority=uc_priority, background_knowledge=background_knowledge,
                        verbose=verbose,
                        show_progress=show_progress, **kwargs)
    else:
        return pc_alg_remake(data=data, node_names=node_names, alpha=alpha, indep_test=indep_test, stable=stable, uc_rule=uc_rule,
        ...
        for x in range(no_of_var):
            ...
            for y in Neigh_x:
                ...
                if background_knowledge is not None and (
                        is_forbidden_remake(background_knowledge,cg.G.nodes[x], cg.G.nodes[y])
                        and is_forbidden_remake(background_knowledge,cg.G.nodes[y], cg.G.nodes[x])):
